...
import org.jeecg.entity.GeneratorTemplatesDO;
import org.jeecg.entity.LoginUser;
import org.jeecg.mapper.GeneratorCodeMapper;
import org.jeecg.service.GeneratorCodeService;
import org.jeecg.service.GeneratorTemplatesService;
import org.jeecg.util.*;
...
import static org.jeecg.util.FileUtil.*;
...
public class GeneratorCodeServiceImpl implements GeneratorCodeService, ApplicationContextAware {
     ...
     *
     * @param fieldNames 所有新字段名
     * @param fieldInfo 所有新增字段信息
     * @param tableName 表名
     * @author: LiKun
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void addFields(List<String> fieldNames, List<Map<String,Object>> fieldInfo, String tableName) {
        StringBuilder sql = new StringBuilder();
        sql.append("ALTER TABLE ").append(tableName);
        // 取出新增字段信息，并拼接成SQL
        for (int i = 0; i < fieldNames.size(); i++) {
            if(fieldInfo.get(i).get("type")==null||("").equals(fieldInfo.get(i).get("type"))){
                log.error(fieldNames.get(i)+MESSAGE_ERROR_TYPE_IS_NULL);
                ServiceUtils.throwException(fieldNames.get(i)+MESSAGE_ERROR_TYPE_IS_NULL);
            }
            int length = 0;
            if(!("").equals(fieldInfo.get(i).get("length"))){
                length = Integer.parseInt((String)fieldInfo.get(i).get("length"));
            }
            String mark = null;
            if(!("").equals(fieldInfo.get(i).get("mark"))){
                mark = (String) fieldInfo.get(i).get("mark");
            }
            String fieldInfoSQL = judgeFieldTypeAndLength((String) fieldInfo.get(i).get("type"),length);
            sql.append(" ADD COLUMN ").append(fieldNames.get(i)).append(" ").append(fieldInfoSQL)
     ...
     *
     * @param updateFieldsJsonArray 更新字段信息JSON
     *        如：{"oldName":{"name":"NEWname","type":"text","mark":"新姓名","length":"32"},{...}}
     * @param tableName   表名
     * @param tablePrefix   表名前缀
     * @param oldList 旧字段名
     * @author: LiKun
     * @Return: List<String> 包含所有更新字段的字段名
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public List<String> updateFields(String updateFieldsJsonArray, String tableName, String tablePrefix, List<String> oldList) {
        ...
        // 取出更新字段信息，并拼接成SQL
            Map<String, Object> updateFieldsJsonToMap = jsonToMap(updateFieldsJsonArray);
            for (Map.Entry<String, Object> m : updateFieldsJsonToMap.entrySet()) {
                ...
                if (("").equals(comment.get("name"))) {
                    log.error(MESSAGE_ERROR_NAME_IS_NULL);
                    ServiceUtils.throwException(MESSAGE_ERROR_NAME_IS_NULL);
                }
                String newName = tablePrefix + comment.get("name");
                if (oldList.contains(newName)) {
                    ServiceUtils.throwException(MESSAGE_ERROR_FIELD_EXIST);
                }
                // 存储更新字段的字段名
                rsList.add(newName);
                if (("").equals(comment.get("type"))) {
                    log.error(comment.get("name") + MESSAGE_ERROR_TYPE_IS_NULL);
                    ServiceUtils.throwException(comment.get("name") + MESSAGE_ERROR_TYPE_IS_NULL);
                }
                String newType = (String) comment.get("type");
                int newLength = 0;
                if (!("").equals(comment.get("length"))) {
                    newLength = Integer.parseInt((String) comment.get("length")) ;
                }
                String newComment = null;
                if (!("").equals(comment.get("mark"))) {
                    newComment = (String) comment.get("mark");
                }
                String fieldInfo = judgeFieldTypeAndLength(newType, newLength);
                // for (Map.Entry<String, Object> n : comment.entrySet()) {
                //     // 拿到新字段名
                //     newName = tablePrefix + n.getValue();
                //     if (oldList.contains(newName)){
                //         ServiceUtils.throwException(MESSAGE_ERROR_FIELD_EXIST);
                //     }
                //     rsList.add(newName);
                ...
                sql.append(" CHANGE COLUMN ").append(tablePrefix).append(m.getKey())
                        .append(" ").append(newName).append(" ").append(fieldInfo).append(" '").append(newComment).append("',");
            }
        sql.setLength(sql.length() - 1);
     ...
     * @param nameToList 字段名链
     * @param markToList 备注链
     * @param lengthToList 长度链
     * @param typeToList 类型链
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public String createTable(String tablePrefix, String tableNamePlus, List<String> nameToList, List<String> markToList, List<Integer> lengthToList, List<String> typeToList){
        ...
        StringBuilder sql = new StringBuilder("create table " + tableNamePlus +
                ...
                "\"主键id(自动生成)\"" + ",");
        // 取字段属性
        for(int j=0;j<nameToList.size();j++){
            String name = nameToList.get(j);
            String type = typeToList.get(j);
            String mark = markToList.get(j);
            int length = lengthToList.get(j);
            // 跳过固有字段，不可重复添加
            if (notAllowed.contains(name.toLowerCase())){
                continue;
            }
            ...
            // 判断字段类型及其长度是否符合要求并拼接成SQL
            String fieldInfo = judgeFieldTypeAndLength(type,length);

            // 加入前缀，防止出现class等关键字问题
            sql.append(tablePrefix).append(name).append(" ").append(fieldInfo)
                    .append("\"").append(mark).append("\"").append(",");
        }
        ...
        log.info(MESSAGE_INFO_CREAT_TABLE + sql);
    ...
    /**
     * @Description: 判断传入的字段长度是否符合要求，并拼接SQL
     * @param maxLength: 该字段支持的最大长度
     * @param type: 字段类型
     * @param length: 传入的字段长度
     * @return java.lang.String SQL
     * @author LiKun
     * @Date 2021/10/22 13:32
     */
    public String splicingFieldInfo(int maxLength,String type,int length){
        StringBuilder fieldTypeAndLength = new StringBuilder();
        // 如果传入的长度大于该字段所支持的最大长度或者小于0，默认赋予长度为其上限
        if(length>maxLength||length<=0){
            fieldTypeAndLength.append(type).append("(").append(maxLength).append(") comment ");
        }else {
            fieldTypeAndLength.append(type).append("(").append(length).append(") comment ");
        }
        return fieldTypeAndLength.toString();
    }

    /**
     * @Description: 判断传入的字段类型及其长度是否符合要求，并返回拼接SQL
     * @param type: 字段类型
     * @param length: 字段长度
     * @return java.lang.String SQL
     * @author LiKun
     ...
     */
    public String judgeFieldTypeAndLength(String type,int length){
        String pattern = "bigint|binary|bit|blob|char|date|datetime|decimal|double|float|geometry|" +
                "geometrycollection|int|integer|json|linestring|longblob|longtext|mediumblob|mediumint|" +
                "mediumtext|multilinestring|multipoint|multipolygon|numeric|point|polygon|smallint|text|time|" +
        ...
        if(m.find()){
            switch (type){
                ...
                case "smallint":
                case "tinyint":
                    fieldInfo = splicingFieldInfo(255,type,length);
                    break;
                case "bit":
                    fieldInfo = splicingFieldInfo(64,type,length);
                    break;
                case "datetime":
                case "time":
                case "timestamp":
                    fieldInfo = splicingFieldInfo(6,type,length);
                    break;
                case "decimal":
                case "numeric":
                    fieldInfo = splicingFieldInfo(65,type,length);
                    break;
                case "varbinary":
                    fieldInfo = splicingFieldInfo(65535,type,length);
                    break;
                case "varchar":
                    fieldInfo = splicingFieldInfo(16383,type,length);
                    break;
                default:
                    fieldInfo = type + " comment ";
                    break;
            }
        }else {
            log.error(MESSAGE_ERROR_TYPE_NOT_EXIST + type);
            ServiceUtils.throwException(MESSAGE_ERROR_TYPE_NOT_EXIST + type);
        }
        return fieldInfo;
    }
    ...
    public boolean cleanAllGenerator(GeneratorCodeDTO generatorCodeVO) {
        try {
            ...
            log.error(e.getMessage());
            ServiceUtils.throwException(MESSAGE_ERROR_CLEAN_DIR);
            return false;
        }
    }
